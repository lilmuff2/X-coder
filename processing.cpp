...
void Processing::ChangeTitle(QString newtitle)
{
    if(newtitle.startsWith("Encode")){
        ui->Run->setText("ENCODE");
        ui->Title->setText("ENCODE");
        Processing::setWindowTitle("ENCODE");
        Processing::Encode = true;
        if(!newtitle.endsWith("Folder")){ui->Input2->setPlaceholderText("OUT FILE");}
        ui->Input1->setPlaceholderText("INPUT FOLDER");
    }
...
void Processing::on_Select1_clicked()
{
    ...
    if(Processing::Encode || Processing::Folder){
       text = QFileDialog::getExistingDirectory(this);
        ...
        #else
            text = QFileDialog::getOpenFileName(this,nullptr,nullptr,tr("SC FILE (*.sc)"));
        #endif
    }
    #if defined Q_OS_ANDROID
...
void Processing::on_Select2_clicked()
{
    ...
    if(Processing::Encode && !Processing::Folder){
        ...
        #else
                text = QFileDialog::getSaveFileName(this,nullptr,nullptr,tr("SC FILE (*.sc)"));
        #endif
    }else{
        text = QFileDialog::getExistingDirectory(this);
    }
    ...
    if(!Processing::Encode && !Processing::Folder){//decoding
        std::thread th(
            [this](){
                ...
                print("Decoding took: "+sc::time::calculate_time(doperation_start, std::chrono::high_resolution_clock::now()));
        ...
        std::thread th(
            [this](){
                ...
                print("Encoding took: "+sc::time::calculate_time(eoperation_start, std::chrono::high_resolution_clock::now()));
                QMetaObject::invokeMethod(ui->Run,"setEnabled", Qt::QueuedConnection,Q_ARG(bool, true));
            }
        );
        th.detach();
        ...
        std::thread th(
            [this](){
                ...
                if(!fs::exists(input_path)){
                    print("ERROR: Input folder doesnt exist!");
                    QMetaObject::invokeMethod(ui->Run,"setEnabled", Qt::QueuedConnection,Q_ARG(bool, true));
                    return;
                }
                if(output_path==""){
                    output_path = input_path.string() + "/Decoded";
                    print("INFO: Out path is empty, so it will be:\n" + output_path.string());
                }
                ...
                print("Decoding all files took: "+sc::time::calculate_time(operation_start, std::chrono::high_resolution_clock::now()));
                QMetaObject::invokeMethod(ui->Run,"setEnabled", Qt::QueuedConnection,Q_ARG(bool, true));
            }
        );
        th.detach();
        ...
        std::thread th(
            [this](){
                ...
                print("Encoding all files took: "+sc::time::calculate_time(operation_start, std::chrono::high_resolution_clock::now()));
                QMetaObject::invokeMethod(ui->Run,"setEnabled", Qt::QueuedConnection,Q_ARG(bool, true));
            }
            );
        th.detach();
...
void Processing::decode(fs::path input_path,fs::path output_path){
    try {
        ...
        if(input_path.stem().string().find("_tex") == std::string::npos && input_path.stem().string().find("_dl") == std::string::npos){
            print("WARNING: INPUT FILE MAY NOT BE _TEX.SC if so out will be empty. INPUT FILE MUST BE TEXTURE FILE");
        }
        print("Working on " + input_path.stem().string() + "...");
        print("Loading file...");
        ...
        fs::create_directories(output_path);
        if(file.isdl){
            print("INFO: It is dl file, so it will be coped to out folder");
